...
export const SubscriptionBoxPremium = ({ color, autoRotate, textureUrl, bgTextureUrl, activeFaces }: Props) => {
  ...
  return (
    <group ref={groupRef} position={[0, -H / 2, 0]}>
      ...
      {/* Inner Walls (Colored) */}
      <mesh scale={0.999} position={[0, H / 2, D / 2 - wall - 0.001]} rotation={[Math.PI, 0, 0]}>
         <planeGeometry args={[W - 4 * wall, H - wall]} />
         <primitive object={innerMat} attach="material" />
      </mesh>
      <mesh scale={0.999} position={[0, H / 2, -D / 2 + wall + 0.001]}>
         <planeGeometry args={[W - 4 * wall, H - wall]} />
         <primitive object={innerMat} attach="material" />
      </mesh>
      <mesh scale={0.999} position={[-W / 2 + wall * 2 + 0.001, H / 2, 0]} rotation={[0, Math.PI / 2, 0]}>
      ...
      <group position={[0, H, -D / 2 + wall / 2]}>
        <group rotation={[lidAngle, 0, 0]}>
          ...
          {/* Front Tuck Flap */}
          <mesh scale={0.999} castShadow material={materials} position={[0, -H / 2 + wall, D - wall]} rotation={[-0.15, 0, 0]}>
            <boxGeometry args={[W - 4 * wall, H - 2 * wall, wall]} />
          </mesh>

          {/* Left Dust Flap */}
          <mesh scale={0.999} castShadow material={materials} position={[-W / 2 + wall, -0.05, 0.2]} rotation={[0, 0, 0.35]}>
