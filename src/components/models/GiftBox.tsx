...
export const GiftBox = ({ color, autoRotate, textureUrl }: Props) => {
  ...
  return (
    <group ref={groupRef}>
      {/* Base box */}
      <mesh scale={0.999} castShadow position={[0, -0.12, 0]}>
        <boxGeometry args={[1.1, 0.85, 1.1]} />
        {mat}
      </mesh>
      {/* Lid (slightly lifted) */}
      <mesh scale={0.999} castShadow position={[0, 0.42, 0]}>
        <boxGeometry args={[1.15, 0.2, 1.15]} />
        {mat}
      </mesh>
      {/* Ribbon vertical strip */}
      <mesh scale={0.999} position={[0, 0.1, 0.56]}>
      ...
      <mesh scale={0.999} position={[0.56, 0.1, 0]}>
      ...
      {[0, Math.PI / 2, Math.PI, -Math.PI / 2].map((angle, i) => (
        <mesh scale={0.999} key={i} position={[Math.sin(angle) * 0.18, 0.58, Math.cos(angle) * 0.18]} rotation={[0, angle, 0.3]}>
          <torusGeometry args={[0.13, 0.035, 8, 20, Math.PI]} />
          <meshStandardMaterial color="#ffd700" roughness={0.2} metalness={0.6} />
        </mesh>
      ))}
      {/* Bow center knot */}
      <mesh scale={0.999} position={[0, 0.6, 0]}>
