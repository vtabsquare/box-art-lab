  ...
  return (
    <group ref={groupRef} position={[-slideOut / 2, 0, 0]}>
      ...
      <group>
        ...
        <mesh scale={0.999} castShadow receiveShadow position={[W/2, H/2, 0]} rotation={[0, 0, -0.6]}>
          <boxGeometry args={[bt, 0.3, D]} />
          {sleeveMat}
        </mesh>
        {/* Front dust flap (folded outwards) */}
        <mesh scale={0.999} castShadow receiveShadow position={[W/2, 0, D/2]} rotation={[0, -0.8, 0]}>
      ...
      <group position={[slideOut, 0, 0]}>
        {/* Tray Base */}
        <mesh scale={0.999} castShadow receiveShadow position={[0, -H/2 + 0.02, 0]}>
        ...
        <mesh scale={0.999} castShadow receiveShadow position={[0, 0, 0]}>
          <boxGeometry args={[W - 0.1, H - 0.08, D - 0.1]} />
          <meshStandardMaterial color="#fafafa" roughness={0.4} />
        </mesh>

        {/* Sealed foil/film on top */}
        <mesh scale={0.999} receiveShadow position={[0, H/2 - 0.03, 0]} rotation={[-Math.PI/2, 0, 0]}>
